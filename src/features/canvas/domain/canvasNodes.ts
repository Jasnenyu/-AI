...
export const VIDEO_GENERATION_NODE_MIN_WIDTH = 280;
export const VIDEO_GENERATION_NODE_MIN_HEIGHT = 320;
...
// 视频生成节点数据
export interface VideoGenerationNodeData extends NodeDisplayData {
  prompt: string;
  model: string;
  aspectRatio: string;
...
export function isVideoGenerationNode(
  node: CanvasNode | null | undefined
): node is Node<VideoGenerationNodeData, typeof CANVAS_NODE_TYPES.videoGeneration> {
  return node?.type === CANVAS_NODE_TYPES.videoGeneration;
}
